...
If Gradle cannot resolve Couchbase dependencies:

- confirm the Couchbase Maven repository is configured
- retry sync
- confirm network access
- ensure corporate proxies/firewalls are not blocking artifact resolution
...
- `geminiKey` is set correctly
- the app was rebuilt after adding the key
- the device has internet access

### Build succeeds but answers are poor

Possible causes:

- sample docs were not ingested yet
- embeddings were not generated
- vector index is missing or placeholder logic is still active
- database layer in the current branch is partially mocked

### App opens but document search appears empty
...
- Main workshop README in the repository root
- `sample_pdfs/README.md` for example prompts and testing ideas
- Couchbase Lite Android documentation
- Gemini Android SDK documentation
